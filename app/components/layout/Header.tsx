        ...
        <nav className="flex-1 overflow-y-auto px-3 py-3">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <div key={item.label}>
                <div
                  className="flex items-center justify-between px-3 py-3.5 rounded-xl hover:bg-gray-50 transition cursor-pointer"
                  onClick={() => (hasSub ? toggle(item.label) : onClose())}
                >
                  <div className="flex items-center gap-2">
                    <Link
                  ...
                  <div className="ml-4 mb-2 border-l-2 border-orange-100 pl-4 space-y-1">
                    {item.simpleDropdown.map((sub) => (
                      <Link
                        ...
                        href="#"
                        onClick={onClose}
                        className="flex items-center justify-between py-2.5 font-medium text-gray-500 hover:text-gray-900 transition"
                        style={{ fontSize: 14 }}
                      >
                        {sub}
                        <IconChevronRight
                ...
                {item.mega && isExpanded && (
                  <div className="ml-4 mb-2 border-l-2 border-orange-100 pl-4">
                    ...
                    <Link
                      ...
                      onClick={onClose}
                      className="flex items-center justify-between py-2.5 font-medium text-orange-500 hover:text-orange-600 transition"
                      style={{ fontSize: 14 }}
                    >
                      Khám phá ngay
                      <IconChevronRight
        ...
        <div className="px-5 py-5 border-t border-gray-100 space-y-3 shrink-0">
          <Link
            ...
            className="flex items-center justify-center gap-2 w-full py-3 rounded-xl
                       ...
                       hover:bg-gray-900 hover:text-white transition"
          >
            <IconLogin size={16} stroke={2} /> Đăng nhập
          </Link>
          <Link
            href="/register"
            onClick={onClose}
            className="flex items-center justify-center gap-2 w-full py-3 rounded-xl
                       bg-orange-500 hover:bg-orange-600 text-sm font-bold text-white transition"
          >
            <IconUserPlus size={16} stroke={2} /> Đăng ký
          </Link>
        </div>
      </div>
    </>
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
        <div className="mx-auto max-w-[1440px] px-4 md:px-8 lg:px-16 flex items-center justify-between h-16 md:h-20">
              ...
              return (
                ...
                >
                  <Link
                    href={item.href}
                    className={`relative flex items-center gap-1.5 px-4 py-2.5 rounded-lg
                      font-semibold tracking-wide transition-colors whitespace-nowrap
                      ${isActive ? "text-gray-950" : "text-gray-500 hover:text-gray-950 hover:bg-gray-50"}`}
                    style={{ fontSize: 15 }}
                  >
                    {item.label}
                    {item.badge && (
                    ...
                    >
                      {item.simpleDropdown.map((sub) => (
                        <Link
                          key={sub}
                          href="#"
                          className="block px-5 py-3 font-medium text-gray-600 hover:text-gray-950 hover:bg-gray-50 rounded-xl transition"
                          style={{ fontSize: 14 }}
                        >
                          {sub}
                        </Link>
          ...
          <div className="flex items-center gap-2 shrink-0 lg:ml-8">
            ...
            {/* ── Cart dropdown (thay thế button cũ) ── */}
            <CartDropdown />
